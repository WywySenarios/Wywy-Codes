...
class TestLogTail:
    @staticmethod
    def url(pipeline, stage_name="RED") -> str:
    ...
    def test_404_for_nonexistent_pipeline(self, client, db, temp_log_root):
        response = client.get(
            "/api/pipelines/00000000-0000-0000-0000-000000000000/logs/RED/"
        )
    ...
    def test_returns_log_entries(self, client, db, pipeline_queued, temp_log_root):
        ...
        self.create_log_file(temp_log_root, pipeline_queued.id, "RED", entries)

        response = client.get(self.url(pipeline_queued, "RED"))
        assert response.status_code == 200
    ...
    def test_includes_orchestrator_log(self, client, db, pipeline_queued, temp_log_root):
        ...
        self.create_log_file(temp_log_root, pipeline_queued.id, "RED", stage_entries)
    ...
    def test_respects_lines_parameter(self, client, db, pipeline_queued, temp_log_root):
        ...
        response = client.get(f"{self.url(pipeline_queued, 'RED')}?lines=3")
        result = response.json()["entries"]
        assert len(result) == 3
